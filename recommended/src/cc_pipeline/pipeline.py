...
from .warc import LocalWarcReader, RemoteWarcReader, S3WarcReader, WarcRecord
...
class PipelineConfig:
    ...
    commoncrawl_bucket: str = "commoncrawl"
    prefer_s3_warc_reads: bool = False
...
class PipelineRunner:
    def __init__(self, config: PipelineConfig) -> None:
        ...
        self.athena = AthenaColumnarClient()
        self.warc_reader = LocalWarcReader()
        self.remote_warc_reader = RemoteWarcReader()
        self.s3_warc_reader = S3WarcReader(default_bucket=config.commoncrawl_bucket)
    ...
    def generate_candidate_manifest_from_athena(self, request: AthenaQueryRequest) -> ExtractionStats:
        if self.candidate_writer is None:
            raise ValueError("candidate_manifest is required for candidate generation")

        stats = ExtractionStats()
        result = self.athena.execute_candidate_query(request)
        for entry in result.to_index_entries():
            stats.candidates_seen += 1
            decision = self.selector.evaluate(entry)
            self._write_candidate_entry(entry=entry, score=decision.score, reasons=decision.reasons)
            if decision.keep:
    ...
    def _read_warc_record(self, filename: str, *, offset: int, length: int) -> WarcRecord:
        ...
        if filename.startswith("s3://"):
            return self.s3_warc_reader.read_record(filename, offset=offset, length=length)
        if self.config.prefer_s3_warc_reads:
            return self.s3_warc_reader.read_record(filename, offset=offset, length=length)
